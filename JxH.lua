...
local St={
    ...
    Settings={
        ...
        AutoFarmLoot=false,KillerSafety=false,AutoEscape=false,AutoHS=false,
    ...
    Cn={autoEscape=nil,killerSafety=nil,noclip=nil,speed=nil,fog=nil,antiAfk=nil,autoRevive=nil,reviveFollow=nil,autoSelfRevive=nil,fly=nil,infiniteJump=nil,watchdog=nil,antiSeat=nil,teamWatch=nil,lootAdded=nil,lootRemoved=nil,hitbox=nil,jumpBoost=nil,autoHS=nil},
    Fl={
        ...
        ghostActive=false,ghostDebounce=false,realChar=nil,fakeChar=nil,ghostSavedStates={},
                legitBotRunning=false, autoHSRunning=false
    },
...
function F.startAutoHS()
    if St.Fl.autoHSRunning then return end
    St.Fl.autoHSRunning = true
    St.Settings.AutoHS = true
    
    local a = Sv.Players
    local b = game:GetService("PathfindingService")
    local c = workspace
    local d = Sv.RunService
    local e = Sv.LocalPlayer
    local f = {Vector3_new(52.5388,264.6762,10.4454),Vector3_new(-37.3878,260.8593,-5.1795),Vector3_new(-25.2802,260.8593,-20.4532),Vector3_new(-18.4071,260.8593,-20.2298),Vector3_new(-18.1125,261.1531,-2.1110),Vector3_new(-12.9700,260.8593,15.2693),Vector3_new(-11.7560,269.0895,-4.7729),Vector3_new(7.0312,260.8593,7.0861),Vector3_new(13.1345,260.8593,-12.2344),Vector3_new(37.4509,260.8593,-0.8731)}
    local g = {AgentRadius=2.5,AgentHeight=5,AgentCanJump=true,AgentCanClimb=true,WaypointSpacing=2.5}
    local h = {}
    local i = Vector3_new()
    local j = tick()
    local k = {}
    ...
    local qR = RaycastParams.new()
    qR.FilterType = Enum.RaycastFilterType.Exclude
    local qO = OverlapParams.new()
    qO.FilterType = Enum.RaycastFilterType.Exclude

    local function u(v)
        if v:IsA("Model") and v.Name=="Trap" then pcall(v.Destroy,v) end
    end
    c.DescendantAdded:Connect(u)

    local function w()
        pcall(function()
            local z = c:FindFirstChild("Space Lab")
            if z then
                local A = z:FindFirstChild("RatTraps")
                if A then for B,C in ipairs(A:GetChildren()) do C:Destroy() end end
            end
        end)
    end
    ...
    local function D(E)
        ...
        local H_ = E:FindFirstChildOfClass("Humanoid")
        if not H_ or H_.Health<=0 then return true end
        local G = E:FindFirstChild("HumanoidRootPart")
        if not G then return false end
        local H = G:FindFirstChild("BleedOutHealth")
        if H and H.Enabled then return true end
        local I = E:FindFirstChild("Downed")
        if I and((I:IsA("BoolValue") and I.Value) or (I:IsA("IntValue") and I.Value>0)) then return true end
        local J = E:FindFirstChild("Incapacitated")
        if J and((J:IsA("BoolValue") and J.Value) or (J:IsA("IntValue") and J.Value>0)) then return true end
        local K = H_:GetState()
        if(K==Enum.HumanoidStateType.FallingDown or K==Enum.HumanoidStateType.Ragdoll) and H_.WalkSpeed<5 then return true end
        return false
    end

    local function L(M)
        local N = M.Team
        if not N then return "lobby" end
        local O = N.Name:lower()
        if O:find("killer") then return "killer" end
        if O:find("survivor") or O:find("innocent") or O:find("hider") then return "survivor" end
        return "lobby"
    end

    local function P()
        for Q,R in ipairs(a:GetPlayers()) do
            if R~=e and L(R)=="killer" and R.Character and R.Character:FindFirstChild("HumanoidRootPart") then
                return R.Character
            end
        end
        return nil
    ...
    local function S(T)
        local U = math.huge
        local V = nil
        local W = e.Character and e.Character:FindFirstChild("HumanoidRootPart")
        if not W then return nil end
        for Q,R in ipairs(a:GetPlayers()) do
            if R~=e and L(R)=="survivor" and R.Character then
                local X = R.Character:FindFirstChild("HumanoidRootPart")
                if X then
                    local Y = D(R.Character)
                    if(T and Y) or (not T and not Y) then
                        local Z = (W.Position-X.Position).Magnitude
                        if Z<U then U=Z;V=R.Character end
                    end
                end
            end
        end
    ...
    local function _getMapAuto()
        for Q,R in ipairs(c:GetChildren()) do
            if(R:IsA("Model") or R:IsA("Folder")) and R~=e.Character then
                local a0 = R.Name:lower()
                if not a0:find("lobby") and not a0:find("spawn") and not a:GetPlayerFromCharacter(R) then
                    if a0:find("map") or R:FindFirstChild("LootSpawns",true) or R:FindFirstChild("Exits",true) then return R end
                end
            end
        end
        return nil
    ...
    local function a1(a2,a3)
        local a4 = _getMapAuto()
        if not a4 then return nil end
        local a5 = a4:FindFirstChild("Exits",true)
        if not a5 then return nil end
        local a6 = -math.huge
        local a7 = nil
        local a8 = tick()
        for Q,R in ipairs(a5:GetChildren()) do
            local a9 = false
            for aa,ab in ipairs(R:GetDescendants()) do
                if ab:IsA("TouchTransmitter") and ab.Parent and ab.Parent.Name=="Trigger" then a9=true;break end
            end
            if a9 then
                local ac = R:FindFirstChild("Bouncer",true)
                if not ac or not ac:IsA("BasePart") or not ac.CanCollide then
                    local ad = R:FindFirstChild("Trigger",true) or R.PrimaryPart
                    if ad and not(h[ad] and a8-h[ad]<15) then
                        local ae = (a2-ad.Position).Magnitude
                        local af = a3 and(a3-ad.Position).Magnitude or math.huge
                        if af>45 then
                            local ag = af-ae
                            if ag>a6 then a6=ag;a7=ad end
                        end
                    end
                end
            end
    ...
    local function ah()
        local a4 = _getMapAuto()
        if not a4 then return nil end
        local ai = a4:FindFirstChild("LootSpawns",true) or c:FindFirstChild("LootSpawns",true)
        ...
        if not W then return nil end
        local a8 = tick()
        for Q,R in ipairs(ai:GetDescendants()) do
            if R:IsA("BasePart") and R.Transparency<0.9 and not(h[R] and a8-h[R]<15) then
                local ae = (W.Position-R.Position).Magnitude
                if ae<a6 then a6=ae;a7=R end
            end
        end
        return a7
    end
    ...
    local function aj(ak)
        ...
        if W and ak then
            pcall(function()
                firetouchinterest(W,ak,0)
                task.wait()
                firetouchinterest(W,ak,1)
            end)
        end
    end

    local function al(am,an,ao)
        local ap = e.Character
        local aq = ap and ap:FindFirstChildOfClass("Humanoid")
        local W = ap and ap:FindFirstChild("HumanoidRootPart")
        if not aq or not W or aq.Health<=0 then return end
        
        qO.FilterDescendantsInstances = {ap}
        local as_ = c:GetPartBoundsInRadius(W.Position,10,qO)
        for Q,R in ipairs(as_) do
            if R.Name:lower():find("door") or R:FindFirstChildWhichIsA("TouchTransmitter") then aj(R) end
        end

        local at = typeof(am)=="Instance" and am.Position or am
        local vDist = m and typeof(m)=="Vector3" and (m-at).Magnitude or 100
        
        if m~=am or ao or vDist>5 then
            local au = b:CreatePath(g)
            local av,aw = pcall(function() au:ComputeAsync(W.Position,at) end)
            if av and au.Status==Enum.PathStatus.Success then
                k = au:GetWaypoints()
                l = 2
                m = am
            else
                if typeof(am)=="Instance" then h[am]=tick() end
                aq:MoveTo(at)
                k = {}
                return
            end
        ...
        if tick()-j>1.5 then
            local ax = Vector3_new(W.Position.X,0,W.Position.Z)
            local ay = Vector3_new(i.X,0,i.Z)
            if(ax-ay).Magnitude<1.5 then
                aq.Jump = true
                aq:Move(Vector3_new(math.random(-1,1),0,math.random(-1,1)))
                if typeof(am)=="Instance" then h[am]=tick() end
                k = {}
                m = nil
            end
        ...
        if #k>0 and l<=#k then
            local az = k[l]
            local ax = Vector3_new(W.Position.X,0,W.Position.Z)
            local aA = Vector3_new(az.Position.X,0,az.Position.Z)
            if(ax-aA).Magnitude<4.5 then l=l+1 end
            if k[l] then
                if k[l].Action==Enum.PathWaypointAction.Jump then aq.Jump=true end
                aq:MoveTo(k[l].Position)
            else
                aq:MoveTo(at)
            end
        else
    ...
    local function aB(aC,aD)
        local aE = aC.Position
        local aF = aD.Position
        local aG = (aE-aF).Unit
        local aH = Vector3_new(aG.X,0,aG.Z).Unit
        local aI = -math.huge
        local aJ = aE
        qR.FilterDescendantsInstances = {e.Character,aD.Parent}
        
        for aL = -90,90,15 do
            local aM = math.rad(aL)
            local aN = CFrame.Angles(0,aM,0)*aH
            local aO = aE+aN*70
            local aP = c:Raycast(aE,aN*70,qR)
            local aQ = aP and aP.Position or aO
            if aP then aQ = aQ-(aN*4) end
            
            local aR = (aQ-aF).Magnitude
            local aS = c:Raycast(aQ,aF-aQ,qR)
            if aS then aR = aR+600 end
            
            if aR>aI then aI=aR;aJ=aQ end
        end
        al(aJ,nil,true)
    end

    local function aT(aU,aV)
        qR.FilterDescendantsInstances = {e.Character,aV.Parent}
        return c:Raycast(aU.Position,aV.Position-aU.Position,qR)==nil
    end

    local function aW()
        local aq = e.Character
        if not aq then return end
        local ar = aq:FindFirstChildOfClass("Humanoid")
        if ar then
            for Q,R in ipairs(aq:GetDescendants()) do
                if R:IsA("Tool") then ar:EquipTool(R) end
            end
            local aX = aq:FindFirstChildOfClass("Tool")
            if aX then aX:Activate() end
        end
    end

    St.Cn.autoHS = d.Heartbeat:Connect(function()
        local aY = tick()
        if aY-o<0.05 then return end
        o = aY
        w()
        
        for Q,R in pairs(h) do if aY-R>15 then h[Q]=nil end end
        ...
        local W = aq and aq:FindFirstChild("HumanoidRootPart")
        if not aq or not ar or not W or ar.Health<=0 then return end

        if aZ=="lobby" then
            if not m or typeof(m)=="Instance" then m=f[math.random(1,#f)] end
            local a_ = Vector3_new(W.Position.X,0,W.Position.Z)
            local b0 = Vector3_new(m.X,0,m.Z)
            if(a_-b0).Magnitude>3 then
                al(m,nil,false)
            else
                ar:MoveTo(W.Position)
                k = {}
                if aY-p>=20 then ar.Jump=true;p=aY end
            end
            return
        end
        ...
        if aZ=="killer" then
            ...
            if b1 and b1:FindFirstChild("HumanoidRootPart") then
                al(b1.HumanoidRootPart.Position,b1.HumanoidRootPart,false)
                if(W.Position-b1.HumanoidRootPart.Position).Magnitude<15 then aW() end
            else
                if not m or typeof(m)=="Instance" or(W.Position-m).Magnitude<5 then
                    m = W.Position+Vector3_new(math.random(-50,50),0,math.random(-50,50))
                end
                al(m,nil,false)
            end
            return
        ...
        if aZ=="survivor" then
            local b2 = P()
            local b3 = b2 and b2:FindFirstChild("HumanoidRootPart")
            local b4 = b3 and(W.Position-b3.Position).Magnitude or math.huge
            local b5 = false
            if b3 and b4<130 then b5=aT(W,b3) end

            if b4<60 or(b4<110 and b5) then
                if n~="FLEE" then n="FLEE";p=0 end
                if aY-p>0.8 then
                    aB(W,b3)
                    p = aY
                end
            ...
            if D(aq) then
                n = "CRAWL"
                local b6 = S(true)
                if b6 and b6:FindFirstChild("HumanoidRootPart") then
                    al(b6.HumanoidRootPart.Position,b6.HumanoidRootPart,true)
                end
                return
            end

            local b7 = a1(W.Position,b3 and b3.Position or nil)
            if b7 then
                n = "EXIT"
                al(b7.Position,b7,false)
                if(W.Position-b7.Position).Magnitude<8 then h[b7]=tick() end
                return
            end
            ...
            if b8 then
                n = "LOOT"
                al(b8.Position,b8,false)
                if(W.Position-b8.Position).Magnitude<7 then
                    aj(b8)
                    h[b8]=tick()
                end
                return
            ...
            n = "WANDER"
            if not m or typeof(m)=="Instance" or(W.Position-m).Magnitude<5 then
                m = W.Position+Vector3_new(math.random(-45,45),0,math.random(-45,45))
            end
            al(m,nil,false)
        end
    end)
...
function F.stopAutoHS()
    if not St.Fl.autoHSRunning then return end
    St.Fl.autoHSRunning = false
    St.Settings.AutoHS = false
    if St.Cn.autoHS then
        St.Cn.autoHS:Disconnect()
        St.Cn.autoHS = nil
    end
end
...
function F.enableFogRemoval()
    ...
    St.OriginalFog.FogEnd=Sv.Lighting.FogEnd
    St.OriginalFog.FogStart=Sv.Lighting.FogStart
    St.OriginalFog.Ambient=Sv.Lighting.Ambient
    St.OriginalFog.OutdoorAmbient=Sv.Lighting.OutdoorAmbient
    St.OriginalFog.ColorShift_Bottom=Sv.Lighting.ColorShift_Bottom
    St.OriginalFog.ColorShift_Top=Sv.Lighting.ColorShift_Top
    St.OriginalFog.ClockTime=Sv.Lighting.ClockTime
    St.OriginalFog.Brightness=Sv.Lighting.Brightness
    St.OriginalFog.GlobalShadows=Sv.Lighting.GlobalShadows
    St.OriginalFog.ExposureCompensation=Sv.Lighting.ExposureCompensation
...
function F.disableFogRemoval()
    ...
    if St.OriginalFog.ExposureCompensation then Sv.Lighting.ExposureCompensation=St.OriginalFog.ExposureCompensation end
        ...
        if type(data.s)=="table" then
            for k,v in pairs(data.s) do 
                if St.Settings[k]~=nil then 
                    if k=="GhostMode" or k=="FlyEnabled" or k=="Noclip" or k=="SpeedEnabled" or k=="JumpBoost" then
                        St.Settings[k]=false
                    else
                        St.Settings[k]=v 
                    end
...
function F.startShiftLock()
    ...
    St.Cn.shiftLock=Sv.RunService.RenderStepped:Connect(function()
...
function F.stopShiftLock()
    St.Settings.ShiftLock=false
...
local _LANG={
    EN={
        ...
        toast_started="Script Loaded  v7.0",
        ...
        tog_farm_loot="Farm Loot", tog_auto_hs="Auto HS (Beta)", tog_auto_revive="Auto Revive", tog_self_revive="Self Revive",
        auto_hs_warn="Disable AutoHS first!",
        auto_hs_confirm="Enabling AutoHS will disable all other Farm and Ghost Mode features. Continue?",
        toast_auto_hs_on="AutoHS: ON",
        toast_auto_hs_off="AutoHS: OFF",
    ...
    RU={
        ...
        toast_started="Скрипт загружен  v7.0",
        ...
        tog_farm_loot="Фарм лута", tog_auto_hs="Auto HS (Бета)", tog_auto_revive="Автоподъём", tog_self_revive="Самоподъём",
        auto_hs_warn="Сначала отключите AutoHS!",
        auto_hs_confirm="Включение AutoHS отключит все остальные функции фарма и режим призрака. Продолжить?",
        toast_auto_hs_on="AutoHS: ВКЛ",
        toast_auto_hs_off="AutoHS: ВЫКЛ",
    ...
    AR={
        ...
        toast_started="تم تحميل السكربت  v7.0",
        ...
        tog_farm_loot="جمع اللوت", tog_auto_hs="Auto HS (تجريبية)", tog_auto_revive="إحياء تلقائي", tog_self_revive="إحياء الذات",
        auto_hs_warn="عطّل AutoHS أولاً!",
        auto_hs_confirm="تفعيل AutoHS سيؤدي إلى إيقاف جميع أوامر الفارم ووضع الشبح. هل تريد المتابعة؟",
        toast_auto_hs_on="AutoHS: تشغيل",
        toast_auto_hs_off="AutoHS: إيقاف",
...
local function buildUI()
    ...
    St.UIRefs._updateStatusBar=function() end
    ...
    local homePage=addTab("home","Main",nil)
    local espPage=addTab("esp","Visuals","esp.png")
    local farmPage=addTab("farm","Auto farm","Farm.png")
    local settPage=addTab("sett","Options","setting.png")
    task.defer(_fitTabs)
    ...
    local function _buildFarm()
        ...
        UI.makeToggle(fGrid1,"AutoHS",_T("tog_auto_hs"),function(on)
            if on then
                local overlay=Instance_new("Frame")
                overlay.Size=UDim2_new(1,0,1,0); overlay.BackgroundColor3=Color3_new(0,0,0)
                overlay.BackgroundTransparency=0.5; overlay.ZIndex=200; overlay.Active=true
                overlay.Parent=ScreenGui
                local dialog=Instance_new("Frame")
                dialog.Size=UDim2_new(0,260,0,110); dialog.Position=UDim2_new(0.5,-130,0.5,-55)
                dialog.BackgroundColor3=UI.C.PANEL; UI.registerTheme(dialog,"PANEL","BackgroundColor3")
                dialog.ZIndex=201; dialog.Parent=overlay
                local dCrn=Instance_new("UICorner"); dCrn.CornerRadius=UDim_new(0,8); dCrn.Parent=dialog
                local dStr=Instance_new("UIStroke"); dStr.Color=UI.C.ACCENT; dStr.Thickness=1; dStr.Parent=dialog
                UI.registerTheme(dStr,"ACCENT","Color")
                local lbl=Instance_new("TextLabel")
                lbl.Size=UDim2_new(1,-20,0,50); lbl.Position=UDim2_new(0,10,0,10)
                lbl.BackgroundTransparency=1; lbl.Text=_T("auto_hs_confirm")
                lbl.TextColor3=Color3_new(1,1,1); lbl.Font=Enum.Font.GothamBold; lbl.TextSize=11
                lbl.TextWrapped=true; lbl.ZIndex=202; lbl.Parent=dialog
                _LR(lbl,"auto_hs_confirm")
                local yesBtn=Instance_new("TextButton")
                yesBtn.Size=UDim2_new(0,100,0,28); yesBtn.Position=UDim2_new(0,20,0,70)
                yesBtn.BackgroundColor3=UI.C.ACCENT; UI.registerTheme(yesBtn,"ACCENT","BackgroundColor3")
                yesBtn.Text=_T("btn_yes"); yesBtn.TextColor3=Color3_new(1,1,1); yesBtn.Font=Enum.Font.GothamBold
                yesBtn.TextSize=10; yesBtn.ZIndex=202; yesBtn.Parent=dialog
                local yCrn=Instance_new("UICorner"); yCrn.CornerRadius=UDim_new(0,6); yCrn.Parent=yesBtn
                _LR(yesBtn,"btn_yes")
                local noBtn=Instance_new("TextButton")
                noBtn.Size=UDim2_new(0,100,0,28); noBtn.Position=UDim2_new(1,-120,0,70)
                noBtn.BackgroundColor3=UI.C.OFF; UI.registerTheme(noBtn,"OFF","BackgroundColor3")
                noBtn.Text=_T("btn_no"); noBtn.TextColor3=Color3_new(1,1,1); noBtn.Font=Enum.Font.GothamBold
                noBtn.TextSize=10; noBtn.ZIndex=202; noBtn.Parent=dialog
                local nCrn=Instance_new("UICorner"); nCrn.CornerRadius=UDim_new(0,6); nCrn.Parent=noBtn
                _LR(noBtn,"btn_no")
                ...
                yesBtn.MouseButton1Click:Connect(function()
                    ...
                    F.startAutoHS()
                    UI.showToast(_T("toast_auto_hs_on"),ScreenGui)
                    overlay:Destroy()
                end)
                noBtn.MouseButton1Click:Connect(function()
                    if St.toggleRefs["AutoHS"] then St.toggleRefs["AutoHS"](false) end
                    St.Settings.AutoHS = false
                    overlay:Destroy()
                end)
            else
                F.stopAutoHS()
                UI.showToast(_T("toast_auto_hs_off"),ScreenGui)
            end
        end,"tog_auto_hs")

        local subF,sllF=UI.makeSubContainer(farmPage)
        UI.makeToggle(fGrid1,"AutoFarmLoot",_T("tog_farm_loot"),function(on)
            if St.Fl.autoHSRunning and on then
                if St.toggleRefs["AutoFarmLoot"] then St.toggleRefs["AutoFarmLoot"](false) end
                St.Settings.AutoFarmLoot = false
                UI.showToast(_T("auto_hs_warn"),ScreenGui)
                return
            end
        ...
        UI.makeToggle(rGrid,"AutoRevive",_T("tog_auto_revive"),function(on)
            if St.Fl.autoHSRunning and on then
                if St.toggleRefs["AutoRevive"] then St.toggleRefs["AutoRevive"](false) end
                St.Settings.AutoRevive = false
                UI.showToast(_T("auto_hs_warn"),ScreenGui)
                return
            end
            if on then
        ...
        UI.makeToggle(rGrid,"AutoSelfRevive",_T("tog_self_revive"),function(on)
            if St.Fl.autoHSRunning and on then
                if St.toggleRefs["AutoSelfRevive"] then St.toggleRefs["AutoSelfRevive"](false) end
                St.Settings.AutoSelfRevive = false
                UI.showToast(_T("auto_hs_warn"),ScreenGui)
                return
            end
        ...
        UI.makeToggle(sGrid,"KillerSafety",_T("tog_killer_safe"),function(on)
            if St.Fl.autoHSRunning and on then
                if St.toggleRefs["KillerSafety"] then St.toggleRefs["KillerSafety"](false) end
                St.Settings.KillerSafety = false
                UI.showToast(_T("auto_hs_warn"),ScreenGui)
                return
            end
        ...
        UI.makeToggle(eGrid,"AutoEscape",_T("tog_auto_escape"),function(on)
            if St.Fl.autoHSRunning and on then
                if St.toggleRefs["AutoEscape"] then St.toggleRefs["AutoEscape"](false) end
                St.Settings.AutoEscape = false
                UI.showToast(_T("auto_hs_warn"),ScreenGui)
                return
            end
    ...
    local function _buildSettings()
        ...
        UI.A(settPage,subSp)
        UI.makeSliderRow(subSp,_T("sl_speed"),16,300,St.Fl.currentSpeed,function(v)
        ...
        UI.makeToggle(mGrid,"GhostMode",_T("tog_ghost_mode"),function(on)
            if St.Fl.autoHSRunning and on then
                if St.toggleRefs["GhostMode"] then St.toggleRefs["GhostMode"](false) end
                St.Settings.GhostMode = false
                UI.showToast(_T("auto_hs_warn"),ScreenGui)
        ...
        do
            ...
            langRow.Size=UDim2_new(1,0,0,34); langRow.BackgroundColor3=UI.C.ROW
            UI.registerTheme(langRow,"ROW","BackgroundColor3")
            langRow.BackgroundTransparency=0.2; langRow.BorderSizePixel=0
            local lrCrn=Instance_new("UICorner"); lrCrn.Parent=langRow; lrCrn.CornerRadius=UDim_new(0,8)
            local rStr=Instance_new("UIStroke"); rStr.Parent=langRow; rStr.Thickness=0.8
            local rGrad=Instance_new("UIGradient"); rGrad.Parent=rStr
            rGrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3_fromRGB(255,255,255)),ColorSequenceKeypoint.new(0.4,Color3_fromRGB(180,200,255)),ColorSequenceKeypoint.new(1,Color3_fromRGB(255,255,255))})
            rGrad.Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,0.85),NumberSequenceKeypoint.new(0.5,0.3),NumberSequenceKeypoint.new(1,0.85)})
            rGrad.Rotation=90

            local lrLbl=Instance_new("TextLabel"); lrLbl.Parent=langRow
            lrLbl.Size=UDim2_new(1,-110,1,0); lrLbl.Position=UDim2_new(0,10,0,0)
            ...
            local langBtn=Instance_new("TextButton"); langBtn.Parent=langRow
            langBtn.Size=UDim2_new(0,90,0,24); langBtn.Position=UDim2_new(1,-96,0.5,-12)
            langBtn.BackgroundColor3=UI.C.OFF; langBtn.Text=""
            ...
            local function _updateLangBtns()
                ...
                langTxt.Text = names[St.Language] or "English"
            end
            _updateLangBtns()
...
local function showChangelog(parentGui)
        ...
        local titleTxt=({EN="✨ What's New in V7.0 ✨",RU="✨ Что нового в V7.0 ✨",AR="✨ الجديد في V7.0 ✨"})[_lng] or "✨ What's New in V7.0 ✨"
...
local function init()
    ...
    Sv.LocalPlayer.CharacterAdded:Connect(function(char)
        ...
        local tt=F.getMyTeamType()
        if tt=="survivor" then
            if St.Settings.AutoHS then F.startAutoHS() end
            if St.Settings.AutoFarmLoot then F.startAutoFarm() end
            if St.Settings.AutoEscape then F.startAutoEscape() end
            if St.Settings.KillerSafety then F.startKillerSafety() end
            if St.Settings.AutoRevive then F.startAutoRevive() end
    ...
    task.spawn(function()
        while task.wait(60) do
            local tt=F.getMyTeamType()
            if tt=="survivor" then
                if St.Settings.AutoHS
                and not St.Fl.autoHSRunning then
                    F.startAutoHS()
                end
